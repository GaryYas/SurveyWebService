...
public class SurveyController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Add new survey, created survey returned",
            ...
            @ApiResponse(responseCode = "404", description = "user for not found",
                    content = @Content),
            @ApiResponse(responseCode = "400", description = "bad parameters for sleep rate or skin condition",
                    content = @Content)})
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Retrieves survey by user id",
                    content = {@Content(mediaType = "application/json",
    ...
    public ResponseEntity<List<SurveyDto>> getSurveysByUser(@RequestParam String userId) {
        List<SurveyDto> allSurveysByUSer = surveyService.getAllSurveysByUser(Integer.parseInt(userId));
        return ResponseEntity.ok(allSurveysByUSer);
    }

    @Operation(summary = "Get survey by survey id", tags = {"survey"})
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Retrieves survey by id",
                    content = {@Content(mediaType = "application/json",
                            schema = @Schema(implementation = SurveyDto.class))}),
            @ApiResponse(responseCode = "500", description = "internal server error",
                    content = @Content),
    ...
    public ResponseEntity<SurveyDto> getSurveysById(@RequestParam String surveyId) {
        return ResponseEntity.ok(surveyService.getSurveyById(Integer.parseInt(surveyId)));
    }

    @Operation(summary = "retrieves all existed surveys", tags = {"survey"})
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "retrieves all surveys",
                    content = {@Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = SurveyDto.class)))}),
            @ApiResponse(responseCode = "500", description = "internal server error",
                    content = @Content)})
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "delete all surveys",
                    content = {@Content(mediaType = "application/json",
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "delete survey by id succeeded",
                    content = {@Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = Integer.class)))}),
            @ApiResponse(responseCode = "500", description = "internal server error",
                    content = @Content),
    ...
    public ResponseEntity<List<Integer>> deleteSurveysByUser(@RequestParam String userId) {
        List<Integer> deletedIds = surveyService.deleteAllSurveysBelongToUser(Integer.parseInt(userId));
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "deleted survey by id",
                    content = {@Content(mediaType = "application/json",
    ...
    public ResponseEntity<Integer> deleteSurveysById(@RequestParam String surveyId) {
        return ResponseEntity.ok(surveyService.deleteSurveyById(Integer.parseInt(surveyId)));
    }
